...
/**
 * Validates the CLIPilot configuration file.
 *
 * Checks that `~/.clipilot/config.json` exists, contains valid JSON,
	...
	try {
		...
		if (code === "ENOENT") {
			return {
				...
				details: "Default configuration will be used. Run 'clipilot config' to create one.",
			};
		}
		if (code === "EACCES") {
			return {
