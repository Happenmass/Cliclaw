...
#### Scenario: 高置信度 completed 走快速通道

- **WHEN** StateDetector 报告 `{ status: "completed", confidence: 0.92 }`
- **THEN** SignalRouter 自动标记当前任务完成，向 MainAgent 对话追加 `[NOTIFY]` 消息

#### Scenario: 低置信度 completed 路由到 MainAgent

- **WHEN** StateDetector 报告 `{ status: "completed", confidence: 0.7 }`
- **THEN** SignalRouter 发送 `[DECISION_NEEDED]` 信号给 MainAgent 进行推理审查
